<!--footer section start-->
<footer class="footer-section" > 
    <!--footer top start-->
    <div class="footer-top gradient-bg">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="row footer-top-wrap">
                      
                        <div class="col-md-4 col-sm-6">
                            <div class="footer-nav-wrap text-white">
                                <h4 class="text-white">UK</h4>
                                <ul class="nav flex-column">
                                    <li class="nav-item mb-5">
                                        International House 24 Holborn Viaduct London EC1A 2BN United Kingdom,                                        
                                    </li>
                                    <li class="nav-item d-flex flex-row align-items-center mb-3">
                                        <img src="{{asset('/landing_assets/img/phone.png')}}" class="mr-3">
                                        <a class="nav-link" href="#">enquiries@oysterchecks.com</a>
                                    </li>
                                    <li class="nav-item d-flex flex-row align-items-center">
                                        <img src="{{asset('/landing_assets/img/phone.png')}}" class="mr-3">
                                        <a class="nav-link" href="#">Office:  +447466588324</a>
                                    </li>                                  
                                   

                                </ul>
                                
                            </div>
                        </div>
                        <div class="col-md-4 col-sm-6">
                            <div class="footer-nav-wrap text-white">
                                <h4 class="text-white">Nigeria</h4>
                                <ul class="nav flex-column">
                                    <li class="nav-item mb-5">
                                        2nd Floor, 1 Adeola Adeoye Street, Off Toyin Street, Ikeja, Lagos Nigeria.                                      
                                    </li>
                                    <li class="nav-item d-flex flex-row align-items-center mb-3">
                                        <img src="{{asset('/landing_assets/img/phone.png')}}" class="mr-3">
                                        <a class="nav-link" href="#">support@oysterchecks.com</a>
                                    </li>
                                    <li class="nav-item d-flex flex-row align-items-center">
                                        <img src="{{asset('/landing_assets/img/phone.png')}}" class="mr-3">
                                        <a class="nav-link" href="#">Office:  +234(1)700-1770</a>
                                    </li> 
                                </ul>                                
                            </div>
                        </div>
                        <div class="col-md-4 col-sm-6">
                            <div class="footer-nav-wrap text-white">
                                <h4 class="text-white">Canada</h4>
                                <ul class="nav flex-column">
                                    <li class="nav-item mb-5">
                                        Balakant Samat,<br> Sri Lanka                                   
                                    </li>
                                    <li class="nav-item d-flex flex-row align-items-center mb-3">
                                        <img src="{{asset('/landing_assets/img/phone.png')}}" class="mr-3">
                                        <a class="nav-link" href="#">Jordy:  +12 (3)456 7890 12</a>
                                    </li>
                                    <li class="nav-item d-flex flex-row align-items-center">
                                        <img src="{{asset('/landing_assets/img/phone.png')}}" class="mr-3">
                                        <a class="nav-link" href="#">Office:  +234(1)700-1770</a>
                                    </li> 
                                </ul>                                
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="container  align-items-center " style="margin-top:50px; padding:30px 30px; background-color:#9797975f; height:150px">
            <div class="row d-flex flex-row" >
                <div class="col-md-5 col-lg-4">
                    <h4 class="text-white">Newsletter Sign Up</h4>
                </div>
                <div class="col-md-6 col-lg-8">
                    <form action="#" method="post" class="subscribe-form subscribe-form-footer subscribe-form-two  ">
                        <div class="d-flex align-items-center">
                            <input type="text" style="border-radius: 10px" class="form-control input" id="email-footer" name="email" placeholder="Enter your email here..." style="background-image: url(&quot;data:image/png;base64,&quot;) !important; background-repeat: no-repeat; background-size: 20px; background-position: 97% center; cursor: auto;" data-temp-mail-org="0">
                            <input type="submit" style="border-radius: 10px"  class="button btn secondary-solid-btn" id="submit-footer" value="Submit">
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
    <!--footer top end-->

    <!--footer copyright start-->
    <div class="footer-bottom  py-2" style="background: #0A142F;">
        <div class="container">
            <div class="row align-items-center justify-content-between">
                <div class="col-md-4 col-sm-6 col-lg-4">
                    <div class="footer-nav-wrap text-white">
                        <h4 class="text-white">      <img src="{{asset('/landing_assets/img/logo-white.png')}}" width="200px" alt="logo" class="img-fluid"/></h4>
                        {{-- <ul class="nav flex-column">
                            <li class="nav-item">
                                <a class="nav-link" href="#">We are an award-winning multinational company. We believe in quality and international standard.</a>
                            </li>                            
                        </ul> --}}
                    </div>
                </div>
                <div class="col-md-5 col-sm-6 col-lg-4">
                    <p class="copyright-text pb-0 mb-0">Copyrights © {{date('Y')}}. All
                        Rights reserved</p>
                </div>
                <div class="col-md-7 col-lg-4 d-none d-md-block d-lg-block">
                    <div class="social-nav text-right">
                        <ul class="list-unstyled social-list mb-0">
                            <li class="list-inline-item tooltip-hover">
                                <a href="#" class="rounded"><span class="ti-facebook text-white"></span></a>
                                <div class="tooltip-item ">Facebook</div>
                            </li>
                            <li class="list-inline-item tooltip-hover"><a href="#" class="rounded"><span class="ti-twitter text-white"></span></a>
                                <div class="tooltip-item">Twitter</div>
                            </li>
                            <li class="list-inline-item tooltip-hover"><a href="#" class="rounded"><span class="ti-linkedin text-white"></span></a>
                                <div class="tooltip-item">Linkedin</div>
                            </li>
                            <li class="list-inline-item tooltip-hover"><a href="#" class="rounded"><span class="ti-dribbble text-white"></span></a>
                                <div class="tooltip-item">Dribbble</div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!--footer copyright end-->
</footer>
<!--footer section end-->

<!--bottom to top button start-->
<button class="scroll-top scroll-to-target" data-target="html">
    <span class="ti-angle-up"></span>
</button>
<!--bottom to top button end-->